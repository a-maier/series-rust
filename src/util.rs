...
pub(crate) fn trim_slice_start<'a, 'b, T>(s: &'a [T], elem: &'b T) -> (&'a [T], usize)
where for<'c> &'c T: PartialEq
{
    let remove = s.iter()
        .enumerate()
        .find(|&(_, c)| c != elem)
...
// number of removed elements
pub(crate) fn trim_slice_end<'a, 'b, T>(s: &'a [T], elem: &'b T) -> (&'a [T], usize)
where for<'c> &'c T: PartialEq
{
    let remove = s.iter().rev()
        .enumerate()
        .find(|&(_, c)| c != elem)
        .map(|(idx, _)| idx)
        .unwrap_or(s.len());
